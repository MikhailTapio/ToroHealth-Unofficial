...
public class HealthBarRenderer {
    ...
    public static void renderInWorld(float partialTick, GuiGraphics graphics, Camera camera) {
        ...
        for (LivingEntity entity : renderedEntities) {
            ...
            render(graphics, entity, 0, 0, FULL_SIZE, true);
    ...
    public static void render(GuiGraphics graphics, LivingEntity entity, double x, double y, float width, boolean inWorld) {
        ...
        if (!inWorld) {
            if (ToroHealth.CONFIG.bar.damageNumberType.equals(Config.NumberType.CUMULATIVE)) {
                drawDamageNumber(graphics, state.lastDmgCumulative, x, y, width);
            } else if (ToroHealth.CONFIG.bar.damageNumberType.equals(Config.NumberType.LAST)) {
                drawDamageNumber(graphics, state.lastDmg, x, y, width);
            }
        }
    }
    ...
    public static void drawDamageNumber(GuiGraphics graphics, int dmg, double x, double y, float width) {
        ...
        graphics.drawString(Minecraft.getInstance().font, s, (int) (x + (width / 2) - sw), (int) y + 5, color);
    }
